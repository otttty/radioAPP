...
const MIN_LINE_GAP_MS = 180; // 短い相槌の後の間
const MAX_LINE_GAP_MS = 550; // 長めのセリフの後の間
const TOPIC_GAP_MS = 900; // トピックが変わるときの一呼吸

/** セリフの長さに応じて自然に感じる間を計算する(短い相槌ほど間を詰める) */
function naturalGapMs(line) {
  const len = line.text.length;
  return Math.min(MAX_LINE_GAP_MS, MIN_LINE_GAP_MS + Math.min(len, 40) * 9);
}
