...
export class ScriptGenerator {
  ...
  async nextSegment(facts) {
    ...
    // 位置取得中(warmup)に作ったセグメントは印を付ける。パイプラインは
    // これを見て、フリートークを先読みしすぎない(位置確定後すぐ本編に移る)。
    ...
    if (this._llm) {
      try {
        ...
        if (Array.isArray(lines) && lines.length >= 2) {
          ...
          return { id: this._id(), topic: plan.topic, factGrounded: plan.factGrounded, lines, warmup };
        }
      } catch (e) {
  ...
  _planSegment(facts) {
    ...
    // つなぐ。オープニングの後は天気やお店には進まず、位置が確定するまで待つ。
    if (facts.warmup) {
    ...
    // お店(レビューお便り)中心の構成。お店を常に優先し、近くのお店を出し切ったときだけ
    // 豆知識を1つ挟む(=豆知識は必要最低限・締め寄りに登場する)。
  ...
  _briefFor(plan, facts) {
    ...
    if (plan.kind === 'weather') {
      ...
      return `${header}\n天気の話題(お便りは使わない。ボブのソロ)。気温は約${Math.round(w.tempC)}度、空模様は「${w.description}」。ここは最低限にとどめる。main 1行だけ、「今日はだいたい${Math.round(w.tempC)}度で${w.description}、〜だぜ」のように気温と天気をサッと言うだけ。長い解説やアドバイスは入れない。数値はこの気温だけを使う。`;
    }
    ...
    if (plan.kind === 'place') {
      ...
      lines.push(
        'お便りには、このお店ならではの具体的な魅力・体験(何を頼んだ・味や雰囲気・どこが良かった・どんな人におすすめ等)をレビューをふくらませて詳しく書く。評価(星・クチコミ件数)はボブの紹介やコメント側で触れてよい。距離・徒歩分はボブが「歩いてすぐだから、みんなも寄ってみて」的に案内する。'
      );
      return lines.join('\n');
    }
    if (plan.kind === 'trivia') {
    ...
    if (facts.warmup) {
      return `${header}\nこれは番組の冒頭、リスナーの現在地を確認している「間つなぎ」のフリートーク(お便りは使わない。全行ボブひとりのトーク)。場所が分かり次第、近くのお店の話に入る予定。今は場所を確かめている最中なので、その待ち時間を、時間帯に合った軽い雑談(音楽の話、今日の気分、リスナーへの語りかけ 等)で楽しくつなぐ。特定の店名・数値・地名は出さない。3〜4行程度。`;
    }
  ...
  _buildWeather(w) {
    return {
      ...
      factGrounded: true,
      lines: [
        { speaker: 'main', text: `今日はだいたい${Math.round(w.tempC)}度で「${w.description}」。さて、いってみましょう。` },
      ],
    };
  }
  ...
  _buildWeatherUnavailable() {
    return {
      ...
      factGrounded: true,
      lines: [
        { speaker: 'main', text: '続いてお天気、と思ったんですが。今日はデータが取れなかったみたい。' },
        { speaker: 'main', text: '適当なことは言えないので、みんな、窓の外をチラッと見てみてください。' },
      ],
    };
  }
  ...
  _buildPlace(place) {
    ...
    const ratingLine = hasRating
      ? `ここ、星${place.rating.toFixed(1)}でクチコミ${place.userRatingCount ?? 0}件の人気店なんですよ。`
      : 'どんなお店かは、行ってからのお楽しみです。';
  ...
  _buildTrivia(t) {
    ...
    return {
      ...
      factGrounded: true,
      lines: [
        { speaker: 'main', text: `ラジオネーム「${name}」さんから、街の小ネタが届いてます。読んでみますね。` },
        { speaker: 'sub', text: `ボブさん、この街の豆知識をひとつ。${t.extract}` },
        { speaker: 'main', text: `いやー、街って面白いですね。${name}さん、ありがとう。` },
      ],
    };
  }
