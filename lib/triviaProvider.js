...
// 面白いご当地ネタ寄りの記事に加点するキーワード(歴史・食・名所・文化)
const POSITIVE_MARKERS = [
  '江戸', '明治', '大正', '昭和', '創建', '創業', '発祥', '由来', '起源', 'ゆかり',
  '名物', '名所', '老舗', '名店', 'グルメ', '料理', '発祥の地', '商店街',
  '神社', '神宮', '寺', '寺院', '城', '城跡', '史跡', '旧跡', '伝説', '逸話',
  '祭', '祭り', '庭園', '公園', '銅像', '記念', '天然記念物', '舞台となった',
  '橋', '坂', '宿場', '関所', '街道', '遺跡', '古墳',
];
...
export async function getNearbyTrivia(lat, lon) {
  ...
  try {
    ...
    /** @type {(import('./types.js').TriviaFact & {_score: number})[]} */
    const scored = pages
      .map((p) => {
        ...
        const pos = countMarkers(extractRaw, POSITIVE_MARKERS);
        const neg = countMarkers(extractRaw, NEGATIVE_MARKERS);
        return {
          title: p.title,
          extract: stripReadings(firstSentences),
          source: `Wikipedia: ${p.title}`,
          _score: pos - neg * 2, // 会社概要系は強めに減点
        };
      })
      .filter((f) => f.extract.length > 0)
      // 面白いネタが1つも無い(=会社概要ばかりの)記事を除外。
      // ポジティブ要素があるか、少なくともネガティブ要素が無いものだけ残す。
      .filter((f) => f._score > 0)
      .sort((a, b) => b._score - a._score);

    /** @type {import('./types.js').TriviaFact[]} */
    const facts = scored.slice(0, 6).map(({ _score, ...rest }) => rest);
