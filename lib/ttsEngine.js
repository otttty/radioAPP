...
export class BrowserTTSEngine {
  ...
  // Web Speech APIは端末内合成でネットワーク往復がないため、先読みは不要(no-op)
  prefetch() {}
  ...
  _voiceProfile(speaker) {
    ...
    if (speaker === 'main') {
      return { voice: pool[0] ?? null, pitch: 1.05, rate: 1.18 };
    }
    // 2つ目の声がなければ同じ声のまま pitch/rate を変えて聞き分けやすくする
    return { voice: pool[1] ?? pool[0] ?? null, pitch: 0.85, rate: 1.12 };
  }
  ...
  speakLine(line, opts = {}) {
    return new Promise((resolve) => {
      ...
      const utter = new SpeechSynthesisUtterance(line.speech ?? line.text);
