...
export class ScriptGenerator {
  ...
  _briefFor(plan, facts) {
    ...
    if (plan.kind === 'place') {
      ...
      // 立ち位置ヒント: 評価とクチコミ数から「穴場/超人気/ふつう」を判定し、
      // ボブの締めコメントがそれに応じて変わる(=毎回テンプレにならない)ようにする。
      const standing = this._placeStanding(p);
      lines.push(`- 立ち位置ヒント(締めコメントの参考にする): ${standing}`);
      lines.push(
        `お便りには、この場所ならではの具体的な魅力・体験(${angle}など)を、1人の実体験としてふくらませて書く。飲食店でなければ「何を食べた」ではなく、その場所に合った体験を書く。`
      );
      lines.push(
        '★締め(最後のmain)はテンプレ厳禁。お便りに実際に出てきた中身への反応を主役にする。星・クチコミ件数・徒歩◯分は毎回律儀に言わないこと(言うとしてもその回で1つだけ、多くの回は言わなくてよい)。立ち位置ヒントは、それに合う回だけ穴場/超人気として軽く触れる。毎回まったく違う締め方にする。'
      );
      return lines.join('\n');
    }
