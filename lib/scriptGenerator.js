...
export class ScriptGenerator {
  ...
  _buildWeather(w) {
    return {
      ...
      factGrounded: true,
      lines: [
        { speaker: 'main', text: `今日はだいたい${Math.round(w.tempC)}度で「${w.description}」ってわけ！ それじゃ、いってみよう！` },
      ],
    };
  }
  ...
  _buildPlace(place) {
    ...
    return {
      ...
      factGrounded: true,
      lines: [
        { speaker: 'main', text: `さあ、お便りいっちゃうぜ！ ラジオネーム「${name}」さんから！ 読んでみるぜ！` },
        { speaker: 'sub', text: mailFirst },
        { speaker: 'sub', text: mailSecond },
        { speaker: 'sub', text: mailThird },
        { speaker: 'main', text: `${name}さん、ありがとう！ ${ratingLine} ここから${distText}、徒歩およそ${mins}分！ 気になった仲間たちは行ってみてくれよな！` },
      ],
    };
  }
  ...
  _buildTrivia(t) {
    ...
    return {
      ...
      factGrounded: true,
      lines: [
        { speaker: 'main', text: `ラジオネーム「${name}」さんから、街の小ネタが届いてるぜ！ 読んでみるぜ！` },
