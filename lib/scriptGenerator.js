...
export class ScriptGenerator {
  ...
  _systemPrompt() {
    return [
      ...
      '',
      '【お便り(ラジオメール)】(speaker="sub")',
      '- この番組は、リスナーから届いたお便りをボブが紹介して進める形式。sub の行は「お便り本文の朗読」を表す。',
      '- お便りの送り主は「いままさにその場所(お店・スポット)から」メールを送っている設定。一人称の体験談として書く(例:「いま〇〇に来てるんですけど」)。',
      '- 本文の内容は素材の事実・利用者の声(雰囲気・評判・体験)に沿わせ、送り主自身が見て感じたこととして自然に語らせる。',
      '- 本文は2〜3ブロックに分けて、それぞれ別の行にする(間にボブの相槌が入る)。書き言葉すぎない、親しみのある自然な文体。「ボブさん」への呼びかけを入れてよい。',
      ...
      '2. sub: お便り本文(前半)。',
      '3. main: ごく短い相槌(「うんうん」「おー、いいねぇ」「わかるよー」など5〜15文字。毎回変える)。',
      '4. sub: お便り本文(続き)。',
      '5. main: 短い相槌かリアクション。',
      '6. (あれば) sub: お便りの締め。',
      ...
      '- 話し言葉で、1行は短めに。全体で6〜8行程度。各行は必ず main か sub。',
  ...
  _briefFor(plan, facts) {
    ...
    if (plan.kind === 'weather') {
      ...
      return `${header}\n天気の話題(お便りは使わない。全行ボブひとりのトーク)。気温は約${Math.round(w.tempC)}度、空模様は「${w.description}」。お出かけの実用アドバイスとして「${weatherAdvice(w.tempC, w.description)}」の要点を自然に織り込む。数値はこの気温だけを使う。`;
    }
    ...
    if (plan.kind === 'place') {
      ...
      if (p.reviews && p.reviews.length) {
        lines.push('- 利用者の声(お便りの送り主自身の体験・感想として織り込む。出典は明かさない):');
        for (const r of p.reviews) lines.push(`  ・${r}`);
      }
      lines.push(
        'お便りには、このお店ならではの魅力・体験を必ず含める。評価(星・クチコミ件数)はボブの紹介やコメント側で触れてもよい。距離・徒歩分はボブが「近くのファンキーな仲間たちも行けるぜ」的に案内する。'
      );
      return lines.join('\n');
    }
    if (plan.kind === 'trivia') {
  ...
  _buildWeather(w) {
    return {
      ...
      factGrounded: true,
      lines: [
        { speaker: 'main', text: 'まずは空模様のチェックだ！' },
        { speaker: 'main', text: `ただいまの気温はおよそ${Math.round(w.tempC)}度、空模様は「${w.description}」！` },
  ...
  _buildPlace(place) {
    ...
    const mailFirst = `ボブさん、こんにちは！ いま「${place.name}」に来てます！`;
    const mailSecond = place.cuisine
      ? `ジャンルは${place.cuisine}。評判どおりのいいお店で、来てよかったです！`
      : '評判どおりのいいお店で、来てよかったです！';
    return {
      id: this._id(),
