...
export class ScriptGenerator {
  ...
  nextSegment(facts) {
    ...
    // お店中心の構成にする: 4本に1本だけトリビアを挟み、残りはお店を優先。
    // (トリビアが多すぎる・神社歴史に偏るという課題への対応)
    this._contentCount += 1;
    const triviaSlot = this._contentCount % 4 === 0;

    const nextPlace = () =>
      this._pickPlace(facts, 'lunch') ||
      this._pickPlace(facts, 'cafe') ||
      this._pickPlace(facts, 'culture');

    if (triviaSlot) {
      const trivia = this._pickTrivia(facts);
      if (trivia) return this._buildTrivia(trivia);
      const place = nextPlace();
      if (place) return this._buildPlace(place);
    } else {
      const place = nextPlace();
      if (place) return this._buildPlace(place);
      const trivia = this._pickTrivia(facts);
  ...
  _buildPlace(place) {
    ...
    // 評価があれば「クチコミの声」を主役の情報にする(なければジャンルで代替)
    ...
    const INTROS = {
      lunch: [
        'お腹の虫が鳴く前に、高評価ランチのご紹介です！',
        'ここでグルメの時間！評判のいいお店、いきましょう！',
        'ごはん選びに迷ってる方へ、おすすめの一軒です！',
        '近くで評判のランチスポット、見つけましたよ！',
      ],
      cafe: [
        'ひと休みしたい方へ、評判のカフェ情報です！',
        'ここでカフェタイムのご提案！人気のお店ですよ！',
        'コーヒーが恋しい時間、おすすめの一軒です！',
        '近くで評判の休憩スポット、見つけました！',
      ],
      culture: [
        'ちょっと寄り道、人気の文化スポットのご案内です！',
        'ここで、街の見どころ情報！',
        '知的なひととき、いかがですか？',
        '近くの評判スポット、ご紹介します！',
      ],
    ...
    return {
      ...
      lines: [
        ...
        {
          speaker: 'main',
          text: hasRating ? `${TIPS[topic] ?? TIPS.lunch}(評価は${place.source}より)` : (TIPS[topic] ?? TIPS.lunch),
        },
      ],
    };
  }
