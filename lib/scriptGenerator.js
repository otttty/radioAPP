...
// つなぎトーク共通の禁止事項。番組の途中に流れるもので、放送はもう始まっている
// ため、毎回挨拶や名乗りから入ると「番組が何度も始まる」ように聞こえてしまう。
const NO_GREETING_RULE =
  '★禁止: 挨拶(「こんばんは」「おはようございます」等)と名乗り(「まちかどラジオのボブです」「お相手はオレ、ボブ」等)、' +
  '番組の紹介は入れない。放送はすでに始まっている途中なので、いきなり話題から入り、そのまま雑談として自然に続ける。';

/** 投稿時刻から「どのくらい前に届いたか」のざっくりした表現を返す */
function elapsedLabel(createdAt) {
  if (!createdAt) return '少し前';
  const min = Math.max(1, Math.round((Date.now() - createdAt) / 60000));
  if (min < 60) return `${min}分ほど前`;
  const hour = Math.round(min / 60);
  if (hour < 24) return `${hour}時間ほど前`;
  const day = Math.round(hour / 24);
  return day <= 1 ? '昨日' : `${day}日ほど前`;
}
...
// 各名前に読み(kana)を持たせる。「息切れ」「別腹」「三杯目」「勝たん」など
// 音声合成が誤読しやすい語が多いため、正しい読みを台本素材でLLMに渡し、
// 読み上げ用の speech に必ずこの読みを使わせる。
// 記号(全角カッコ等)は不自然な間の原因になるので名前には入れない。
const RADIO_NAMES = [
  { name: '回転寿司で酔うタイプ', kana: 'かいてんずしでようタイプ' },
  { name: '坂道でいつも息切れ', kana: 'さかみちでいつもいきぎれ' },
  { name: '地図アプリに逆らう男', kana: 'ちずアプリにさからうおとこ' },
  { name: 'コーヒー三杯目のジャズ好き', kana: 'コーヒーさんばいめのジャズずき' },
  { name: '財布を家に忘れた常連', kana: 'さいふをいえにわすれたじょうれん' },
  { name: '深夜のプリン強盗', kana: 'しんやのプリンごうとう' },
  { name: '定食屋で優柔不断', kana: 'ていしょくやでゆうじゅうふだん' },
  { name: '傘を置き忘れる名人', kana: 'かさをおきわすれるめいじん' },
  { name: '餃子は飲み物', kana: 'ぎょうざはのみもの' },
  { name: '昼寝の天才', kana: 'ひるねのてんさい' },
  { name: 'レシート集めがち', kana: 'レシートあつめがち' },
  { name: '隣の駅まで歩く派', kana: 'となりのえきまであるくは' },
  { name: 'メニュー全部気になる係', kana: 'メニューぜんぶきになるかかり' },
  { name: '窓側の席しか勝たん', kana: 'まどがわのせきしかかたん' },
  { name: 'スープまで飲み干す派', kana: 'スープまでのみほすは' },
  { name: '路地裏コンパス', kana: 'ろじうらコンパス' },
  { name: 'いつも小銭が足りない', kana: 'いつもこぜにがたりない' },
  { name: '食後のデザート別腹担当', kana: 'しょくごのデザートべつばらたんとう' },
  { name: '散歩のつもりが遠足', kana: 'さんぽのつもりがえんそく' },
  { name: '推しは商店街', kana: 'おしはしょうてんがい' },
];
...
export class ScriptGenerator {
  ...
  constructor(opts = {}) {
    ...
    this._lastWasCommunityMail = false; // 他リスナーのお便りを連続させないための印
  ...
  _planSegment(facts) {
    ...
    // この街に以前届いた「他のリスナーからのお便り」。スポット紹介と交互に出す
    // (連続させると投稿ばかりの番組になってしまうため)。
    if (!this._lastWasCommunityMail) {
      const community = facts.takeCommunityMail?.();
      if (community) {
        this._lastWasCommunityMail = true;
        return { kind: 'communityMail', topic: 'mail', factGrounded: true, userMail: community };
      }
    }
    this._lastWasCommunityMail = false;
  ...
  _systemPrompt() {
    return [
      ...
      '- 本文は2ブロックちょうどに分け、それぞれ別の sub 行にする(段落の区切り)。3ブロック以上にはしない。1通のお便りとして長くなりすぎないよう、各ブロックは2〜3文程度にまとめる。ブロック間は同じ人物の話の流れとして自然につながるようにする。',
      ...
      '- ★ラジオネームの読み: 素材に読みが指定されている場合、そのラジオネームを含む行の speech では、指定された読みを一字一句そのまま使う(自分で読みを作り直さない)。ラジオネームは造語や言葉遊びが多く誤読しやすいため、ここは特に厳密に守る。',
      '',
      '【1トピックの構成】(お便りの回)',
      '1. main: 導入。話題をチラ見せし、「ラジオネーム『〇〇』さんからのお便り!」と送り主を紹介して「読んでみるぜ」的に振る。',
      '2. sub: お便り本文(前半。状況と第一印象。2〜3文)。',
      '3. sub: お便り本文(後半。具体的なディテールとおすすめポイント。2〜3文)。※sub はこの2つだけ。3つ目の sub は作らない。',
      '4. main: 読み終わったボブの締めコメント。締めの主役は「そのお便りの中身への感情の乗った反応」= 送り主が語った料理・景色・エピソードを名指しで受けて、うらやましがる・本気で誉める・共感する・行きたくなる・軽く笑う、のいずれか。2〜4文かけてしっかり語り、お便りの具体的なディテールを最低1つ引用して、それへの自分の感想をちゃんと述べる(薄い一言で流さない)。「〇〇さん、ありがとう!」は添えても添えなくてもよい。',
      ...
      '- 2つの sub(お便り本文)の合間には、前述の短い相槌・感嘆を1回だけ入れてよい(ただし sub を挟まず main を連続はしない)。それ以外はボブは黙って読む。main がしっかり入るのは導入(1)と締め(4)。',
      '- 全体の行数は、導入1行 + sub 2行 + 締め1行 = 4行が基本(相槌を入れる回でも5行まで)。これより長くしない。',
      '- 素材に「お便りは使わない」とある場合は、全行 main のボブひとりのトークにする。',
      '- 素材が「リスナーから“いま実際に届いた”お便りの回」の場合は、上の1〜4の構成ではなく素材に書かれた作り方に従う。お便り本文は創作せず、渡された文章をそのまま読み上げ、その内容に具体的に答えることを最優先にする。',
      '',
      '【マンネリ禁止】',
      ...
      '- 話し言葉で。お便り本文(sub)は具体的に、ボブの相槌(main)は短く。1トピックは全体で4〜5行に収める(長くしない)。各行は必ず main か sub。',
  ...
  _briefFor(plan, facts) {
    ...
    // userMail = いま聴いている本人の投稿 / communityMail = 以前この街に届いた他の人の投稿
    if (plan.kind === 'userMail' || plan.kind === 'communityMail') {
      const m = plan.userMail;
      const isCommunity = plan.kind === 'communityMail';
      // 投稿者が名乗った名前には読みが無いので、その場合はLLMに読みを判断させる
      const picked = m.radioName ? { name: m.radioName, kana: null } : this._nextRadioName();
      const name = picked.name;
      const lines = [
        header,
        isCommunity
          ? `この街に以前届いていた、別のリスナーからのお便りの回(${elapsedLabel(m.createdAt)}に届いたもの)。下のお便り本文を、ボブが番組で読み上げて、その内容にきちんと答える。`
          : 'リスナーから“いま実際に届いた”お便りの回。下のお便り本文を、ボブが番組で読み上げて、その内容にきちんと答える。',
        picked.kana
          ? `- ラジオネーム: 「${name}」(この名前をそのまま使う。読みは「${picked.kana}」で、speechには必ずこの読みを使う)`
          : `- ラジオネーム: 「${name}」(この名前をそのまま使う。読み方は文脈から正しく判断し、speechではかなに開く)`,
        '- お便り本文(★これは実際に送られてきた文章。内容を勝手に作り替えない。要点は省略せず読み上げる):',
        `  「${m.body}」`,
        '',
        '【この回の作り方】',
        isCommunity
          ? `1. main: 「この街に届いてたお便りを紹介するぜ。ラジオネーム『${name}』さんから」と、以前届いたものだと分かるように紹介して読み上げに入る。`
          : `1. main: 「お便りが届いてます!ラジオネーム『${name}』さんから」と紹介して読み上げに入る。`,
        '2. sub: お便り本文を読み上げる(長ければ2〜3ブロックに分けてよいが、内容は変えない。誤字は自然に直して読んでよい)。',
        '3. main: ★ここが最重要。お便りの中身に“ちゃんと答える”。質問されていれば答える、感想や報告なら受け止めて共感し、具体的に反応する。一般論でごまかさず、書かれている固有の内容に触れること。ただし長くしすぎない: 2〜3文で、要点だけを的確に返して締める。',
        '- 事実を知らないことを聞かれたら、知ったかぶりせず正直に「オレも詳しくないんだけど」と前置きして、感想や視点を返す。周辺スポットの創作(存在しない店名など)は禁止。',
        '- 上の「いまの空気感」も踏まえて、時間帯や季節に合った一言を自然に添えてよい。',
      ];
      lines.push(
        isCommunity
          ? '- 送り主はいまこの場にいるとは限らない(以前ここから送ってくれた人)。「同じ場所にいる人に届けるつもりで」いま聴いているリスナーにも語りかけて締める。'
    ...
    if (plan.kind === 'place') {
      ...
      const lines = [
        ...
        `- 種類: ${kind}`,
        (() => {
          const rn = this._nextRadioName();
          return `- 今回のラジオネーム: 「${rn.name}」(一字一句そのまま使う。読みは「${rn.kana}」で、speechには必ずこの読みを使う)`;
        })(),
      ];
      ...
      if (reviews.length) {
        ...
        // 参考に渡すのは1件だけ(軸と合わせて2件)。3件だと情報を詰め込みすぎて
        // 「複数人のレビューの寄せ集め」に戻ってしまうため。
        const others = reviews.filter((_, i) => i !== axisIdx).slice(0, 1);
        if (others.length) {
  ...
  _templateSegment(plan, facts) {
    switch (plan.kind) {
      ...
      case 'communityMail':
        return this._buildUserMail(plan.userMail, true);
      case 'awaiting':
   ...
   */
  _nextRadioName() {
    let unused = RADIO_NAMES.filter((n) => !this._usedNamePool.has(n.name));
    if (unused.length === 0) {
      this._usedNamePool.clear();
      unused = RADIO_NAMES.slice();
    }
    const picked = unused[Math.floor(this.rng() * unused.length)];
    this._usedNamePool.add(picked.name);
    this._usedRadioNames.push(picked.name);
    return picked; // { name, kana }
  }
  ...
  _buildOpening(facts) {
    ...
    // オープニングの直後は、位置情報がまだ確定していないため必ずフリートークが挟まる。
    // ここで「さっそくお便りいってみましょう」と振ると、次に来るのが雑談で肩透かしに
    // なるので、お便りに繋げず番組の入りとして締めるだけにする。
    const bridge = pick(
      [
        `時刻はちょうど${hour}時くらい。今日もゆるっとお付き合いください。`,
        `いま${hour}時ごろですかね。肩の力を抜いていきましょう。`,
        `${hour}時台、いい時間です。のんびりやっていきますよ。`,
      ],
      this.rng
    );
    return {
  ...
  _buildPlace(place) {
    ...
    const name = this._nextRadioName().name;
    // お便り本文は2ブロックまで(LLM経路と同じ長さに合わせる)
    const mailFirst = place.summary
      ? `ボブさん、こんにちは。いま「${place.name}」に来ています。${place.summary}`
      : `ボブさん、こんにちは。いま「${place.name}」に来ています。${place.cuisine ? `${place.cuisine}で、雰囲気もよくて居心地がいいです。` : '雰囲気もよくて、居心地がいいです。'}`;
    const mailSecond = place.reviews && place.reviews.length
   ...
   */
  _buildUserMail(mail, isCommunity = false) {
    const name = mail.radioName || this._nextRadioName().name;
    const intro = isCommunity
      ? `この街に届いてたお便りを紹介するぜ。${elapsedLabel(mail.createdAt)}、ラジオネーム「${name}」さんから。`
      : `お、お便りが届いてます!ラジオネーム「${name}」さんから。さっそく読んでみますね。`;
    const outro = isCommunity
      ? `${name}さん、ありがとう。同じ場所にいるみんなにも、ちゃんと届けましたよ。`
      : `${name}さん、お便りありがとう!聴きながら送ってくれたんですね、うれしいなあ。しっかり受け取りました。またいつでも送ってください。`;
    return {
      id: this._id(),
      topic: 'mail',
      factGrounded: true,
