...
export class ScriptGenerator {
  ...
  _systemPrompt() {
    return [
      ...
      '- ボブは基本お便りを読み上げることに徹する。ただし、お便りの区切り(sub と sub の間)で、ごくたまに短い感嘆・手短な返答を main の1行として挟んでよい(例:「あ〜、それいいですね〜」「へえ、最高じゃん」「うわ、それは気になる」)。※挟むのは1通につき多くて1回まで。連続では絶対に挟まない。多すぎは絶対NG。挟まない回があってよい(むしろそれが普通)。基本は最後まで読んでからまとめてコメントする。',
      ...
      '5. main: 読み終わったボブの締めコメント。「〇〇さん、ありがとう!」+お便りへの反応+距離やアクセスへの一言。ここはテンプレ厳禁で毎回はっきり内容を変える。以下のどれか(または組み合わせ)を、その回の素材に合わせて選ぶ: (a) お便りに出てきた具体的な中身(頼んだもの・見た景色・エピソード)に触れて感想を言う、(b) 素材の「立ち位置ヒント」に従って「知る人ぞ知る穴場」なら穴場として、「みんな大好きな超人気スポット」なら人気ぶりに触れてコメントする、(c) 時間帯やリスナーの状況に絡めて誘う。毎回まったく違う言い回し・語順・締め方にする。',
      '- 原則、2〜4の sub(お便り本文)の合間に main は入れず、ボブは黙って読む。例外として、ごくたまに1回だけ短い感嘆(前述)を挟むのは可。main がしっかり入るのは導入(1)と読み終わりの締め(5)。',
  ...
  _briefFor(plan, facts) {
    ...
    if (plan.kind === 'place') {
      ...
      lines.push(
        '★締め(最後のmain)はテンプレ厳禁。上の「立ち位置ヒント」と、お便りに実際に出てきた具体的な中身の両方をふまえて、毎回まったく違う言い回しでコメントする。'
      );
      return lines.join('\n');
    }
