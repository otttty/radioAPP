...
// 記事冒頭の「タイトル（かな読み…）」から、タイトルの正しい読みを取り出す。
// 音声では復唱させないが、読み上げ用かな変換(LLM)への読みヒントとして使う
// (例: 大手町=おおてまち、将門塚=しょうもんづか のような誤読しやすい固有名詞対策)。
function extractTitleReading(title, extractRaw) {
  try {
    const m = extractRaw.match(
      new RegExp(`${escapeRegExp(title)}\\s*[（(]([぀-ヿー・\\s]+?)(?:[、,][^）)]*)?[）)]`)
    );
...
export async function getNearbyTrivia(lat, lon) {
  ...
  try {
    ...
    // 豆知識は「最後の1トピックに入れるくらい」の頻度なので、500mだとほぼ拾えない
    // 地域が多い。たまに1つ拾える程度に、ここだけ少し広め(1000m)にしている。
    const geoUrl = `${WIKI_ENDPOINT}?action=query&list=geosearch&gscoord=${lat}%7C${lon}&gsradius=1000&gslimit=20&format=json&origin=*`;
    ...
    const scored = pages
      .map((p) => {
        ...
        return {
          title: p.title,
          reading: extractTitleReading(p.title, extractRaw),
