...
export class ScriptGenerator {
  ...
  _closingDirective(p) {
    ...
    // どの回でも使える基本スタイル(いずれも“数値の羅列”をしない)
    const styles = [
      'お便りに出てきた具体的な中身(頼んだもの・見た景色・その人のエピソード)への「生の感想」だけで締める。うらやましがる/笑う/共感する等。星の数・クチコミ件数・徒歩の分数には一切触れない。',
      'リスナー全体への呼びかけや軽い問いかけで締める(「これ好きな人、絶対いるでしょ」「今日ちょっと寄ってみたら?」など)。数値情報は入れない。',
      `いまが約${hour}時、という時間帯やシーンに絡めて、さらっと誘って締める。星やクチコミ件数は言わない。`,
      'ごく短く、余韻を残して一言でさらっと締める(長々と説明しない)。数値は入れない。',
      'お便りの中の一番おいしい/印象的なポイントを一つ拾って、そこだけ膨らませて「いいなあ」と締める。星・件数・徒歩は言わない。',
      '「行きやすさ」の雰囲気だけ一言添えて締める(歩いてすぐ、ふらっと寄れる、など。正確な徒歩◯分や星・件数の“数字”は出さない)。',
    ];
    // 立ち位置がはっきりしている回だけ、そのネタを締めに使える選択肢を足す
    if (isGem) {
      styles.push('この場所が「知る人ぞ知る穴場」だという立ち位置に触れて、あまり知られてない特別感・お得感で締める。徒歩や件数の羅列はしない。');
    }
    if (isPopular) {
      styles.push('この場所が「みんな知ってる超人気スポット」だという立ち位置に触れて、定番感・人気ぶり(混み具合など)で締める。星や徒歩の羅列はしない。');
    }
    const chosen = pick(styles, this.rng);
    return `【今回の締め方(必ずこの1方針だけで締める。他の型・数値の羅列は使わない)】${chosen}「〇〇さん、ありがとう」は付けても付けなくてもよいが、直近の回と同じ言い回しにはしない。`;
  }
  ...
  _systemPrompt() {
    return [
      ...
      '5. main: 読み終わったボブの締めコメント。まず何より「そのお便りの中身への生の反応」を軸にする(送り主が語った料理・景色・エピソードを受けて、うらやましがる・笑う・共感する・行きたくなる、など)。「〇〇さん、ありがとう!」は添えてよい。',
      '   ★締めのテンプレ化を厳禁とする。星の数・クチコミ件数・徒歩◯分を「毎回ぜんぶ律儀に言う」のは絶対にやめる。これらの情報は、その回で言いたくなった時だけ1つ拾う程度にし、多くの回はどれも言わなくてよい。何を入れて何を省くかを毎回変える。締めの語順・長さ・トーンも毎回まるごと変え、2回として同じ型にしない。',
      '   例のバリエーション(これらに限らない):ある回は距離に触れず中身の感想だけ/ある回は「知る人ぞ知る穴場ですよ」と立ち位置だけ/ある回は「超人気店だから並ぶ覚悟で」と人気ぶりだけ/ある回は時間帯に絡めて「小腹すいた人ぜひ」/ある回はリスナーへの問いかけで締める、など。立ち位置ヒントがあれば、それに合う回だけ穴場/超人気に触れる。',
  ...
  _briefFor(plan, facts) {
    ...
    if (plan.kind === 'place') {
      ...
      // 立ち位置ヒント: 評価とクチコミ数から「穴場/超人気/ふつう」を判定。
      const standing = this._placeStanding(p);
      lines.push(`- 立ち位置ヒント: ${standing}`);
      lines.push(
  ...
  _buildPlace(place) {
    ...
    // 立ち位置(穴場/超人気/ふつう)で締めの一言を変え、毎回テンプレにならないようにする
    let ratingLine;
    if (!hasRating) {
      ratingLine = 'どんな場所かは、行ってからのお楽しみです。';
    } else if (n >= 3000) {
      ratingLine = `ここ、星${place.rating.toFixed(1)}でクチコミ${n}件超えの、もうみんな知ってる超人気スポットですね。`;
    } else if (place.rating >= 4.3 && n <= 500) {
      ratingLine = `星${place.rating.toFixed(1)}なのにクチコミ${n}件。これはまさに知る人ぞ知る穴場ですよ。`;
    } else {
      ratingLine = `星${place.rating.toFixed(1)}、クチコミ${n}件。評判のいい良店ですね。`;
    }
